...
`
	...
	if err := tx.GetContext(ctx, &hitSpam, spamCheckQuery, livestreamModel.UserID, livestreamModel.ID, req.Comment); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to check for spam: "+err.Error())
	}
...
func moderateHandler(c echo.Context) error {
	...
	//		WHERE
	//			id = ? AND livestream_id = ? AND
	//			(SELECT COUNT(*)
	//				FROM (SELECT ? AS text) AS texts
	//				INNER JOIN (SELECT CONCAT('%', ?, '%')	AS pattern) AS patterns
	...
	INNER JOIN ng_words ng ON lc.livestream_id = ng.livestream_id
	WHERE ng.livestream_id = ? AND lc.comment LIKE CONCAT('%', ng.word, '%');
	`
	if _, err := tx.ExecContext(ctx, deleteQuery, livestreamID); err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to delete old livecomments that hit spams: "+err.Error())
	}
